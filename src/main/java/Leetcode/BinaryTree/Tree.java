...
public class Tree {
    ...
    */
     ...
     * 接着再记录这四个方向上
     * 符合条件的像素点与他们对应四个方向上像素值与初始坐标相同的相连像素点，……，重复该过程。
     * 将所有有记录的像素点的颜色值改为新的颜色值。
     *
     * 最后返回经过上色渲染后的图像。
     *
     * solution: 这题真读不懂，读懂了 解法肉眼看起来挺简单的  实现起来可能有一定难度
     * image 二维数组其实就是一个矩阵，(sr,sc) 就是矩阵中的坐标，最好本子上画一个图
     * 若(sr,sc)对应的color和newColor一样，就不做变动 否则替换掉
     * 并且把相邻的符合条件的color也变成newColor，一只延伸，直到没有符合的条件返回    image   符合条件 ，
     * 指在棋盘类 且color ！= newColor
